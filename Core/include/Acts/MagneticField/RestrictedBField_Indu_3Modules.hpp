...
class RestrictedBField_Indu_3Modules final : public MagneticFieldProvider {
 public:
  struct Cache {
  ...
  RestrictedBField_Indu_3Modules(const Vector3& field)
      : m_BField(field),
        m_magVolume(std::make_shared<TrackingVolume>(
            Transform3(Translation3{13600., 0., 0.}*AngleAxis3(3.14*0.5, Vector3(0., 1., 0.))),
            std::static_pointer_cast<VolumeBounds>(
                std::const_pointer_cast<CylinderVolumeBounds>(
                    std::make_shared<const CylinderVolumeBounds>(0.0, 990., 2100.))),
            "MagneticFieldVolume")) {}
