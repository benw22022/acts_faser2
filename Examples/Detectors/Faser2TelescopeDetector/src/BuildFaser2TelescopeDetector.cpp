...
ActsExamples::Faser2Telescope::buildDetector(
    const typename ActsExamples::Faser2Telescope::Faser2TelescopeDetectorElement::
        ContextType& gctx,
    std::vector<
        std::shared_ptr<ActsExamples::Faser2Telescope::Faser2TelescopeDetectorElement>>&
        detectorStore,
    const std::vector<double>& positions,
    const std::vector<double>& stereoAngles,
    const std::array<double, 2>& offsets, const std::array<double, 2>& bounds,
    double thickness,const std::vector<double>& boundsX, const std::vector<double>& boundsY,const std::vector<int>& Type_surf, ActsExamples::Faser2Telescope::Faser2TelescopeSurfaceType surfaceType,
 ...
 */
  ...
  Acts::Material SciFiber = Acts::Material::fromMassDensity(
      42.20_cm, 90.2_cm, 12.0855, 6, 1.18_g / 1_cm3);
  Acts::MaterialSlab matProp(SciFiber, thickness);
  ...
  // Validate boundsX and boundsY
  bool useDefaultBounds = boundsX.empty() || boundsY.empty() || 
                          boundsX.size() != positions.size() || 
                          boundsY.size() != positions.size();

  // Calculate maximum bounds
  double maxBoundX = useDefaultBounds ? bounds[0] : *std::max_element(boundsX.begin(), boundsX.end());
  double maxBoundY = useDefaultBounds ? bounds[1] : *std::max_element(boundsY.begin(), boundsY.end());
  ...
  for (unsigned int i = 0; i < nLayers; ++i) {
    ...
    const auto pBounds = useDefaultBounds ?
        std::make_shared<const Acts::RectangleBounds>(bounds[0], bounds[1]) :
        std::make_shared<const Acts::RectangleBounds>(boundsX[i], boundsY[i]);
    // The radial bounds for disc surface
    const auto rBounds = useDefaultBounds ?
        std::make_shared<const Acts::RadialBounds>(bounds[0], bounds[1]) :
        std::make_shared<const Acts::RadialBounds>(boundsX[i], boundsY[i]);

    Faser2TelescopeSurfaceType current_surfaceType = static_cast<ActsExamples::Faser2Telescope::Faser2TelescopeSurfaceType>(Type_surf[i]);

    // Create the detector element
    std::shared_ptr<Faser2TelescopeDetectorElement> detElement = nullptr;
    if (current_surfaceType == Faser2TelescopeSurfaceType::Plane) {
      detElement = std::make_shared<Faser2TelescopeDetectorElement>(
  ...
  if (surfaceType == Faser2TelescopeSurfaceType::Plane) {
    boundsVol = std::make_shared<Acts::CuboidVolumeBounds>(
        maxBoundX + 5._mm, maxBoundY + 5._mm, length + 10._mm);
  } else {
    boundsVol = std::make_shared<Acts::CylinderVolumeBounds>(
        std::max(maxBoundX - 5.0_mm, 0.), maxBoundY + 5._mm, length + 10._mm);
  }
  ...
  auto trackVolume = std::make_shared<Acts::TrackingVolume>(
      ...
      Acts::MutableTrackingVolumeVector{}, "Faser2Telescope");
