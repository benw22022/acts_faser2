...
if "__main__" == __name__:
    ...
    Z_axis = 2
    thick_layer = 4
    bounds_dim=[500,1500]   #Half length in X and Y in mm
    Stereo_angles = [0] * n_layers
    
    ## Tracking geometry detector 
    detector, trackingGeometry, decorators = acts.examples.TelescopeDetector.create(
        bounds=bounds_dim, positions=pos, binValue=X_axis,thickness=thick_layer,stereos=Stereo_angles,
    )
    
    srcdir = Path(__file__).resolve().parent.parent.parent.parent
    
    
    inputParticlePath = Path("./Output_ttk/bisddf/particles.root")
    ...
    #inputParticlePath = Path("/exp/atlas/salin/Faser2/acts__F2/HepMC_Root/Root_LLP/Root_DarkHiggs_X_good/Particles_DarkHiggs_m0.8128_mu_mu.root")
