...
void addDetector(Context& ctx) {
  ...
  {
    using Faser2TelescopeDetector = Faser2Telescope::Faser2TelescopeDetector;
    using Config = Faser2TelescopeDetector::Config;

    auto F2td =
        py::class_<Faser2TelescopeDetector, std::shared_ptr<Faser2TelescopeDetector>>(
            mex, "Faser2TelescopeDetector")
            .def(py::init<>())
            .def("finalize",
                 py::overload_cast<
                     const Config&,
                     const std::shared_ptr<const Acts::IMaterialDecorator>&>(
                     &Faser2TelescopeDetector::finalize));
